...
<jsp:directive.page import="core.org.akaza.openclinica.bean.core.SubjectEventStatus"/>
...
<jsp:useBean scope="request" id="currEvent" class="core.org.akaza.openclinica.bean.managestudy.StudyEventBean" />
<jsp:useBean class="org.apache.commons.lang.StringEscapeUtils" id="esc" />

<%--
<jsp:useBean scope="request" id="eventStatus" class="core.org.akaza.openclinica.bean.core.SubjectEventStatus" />
--%>
...
<div id="Lock_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>" style="position: absolute; visibility: hidden; z-index: 3; width: 50px; height: 30px; top: 0px;">
	 ...
	 <c:otherwise>
	   	<a href="javascript:leftnavExpand('Menu_on_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>'); 
		     ...
		     onmouseover="layersShowOrHide('visible','Event_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>');
				...
				javascript:setImage('ExpandIcon_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>','images/icon_blank.gif');"><img src="images/spacer.gif" border="0" height="30" width="50"></a>
			  
</div>
		
	<div id="Event_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>" style="position: absolute; visibility: hidden; z-index: 3;width: <c:out value="${divWidth}"/>px; top: 0px; float: left;">
    ...
    <div class="box_TR"><div class="box_BL"><div class="box_BR">
		        ...
		        <c:when test="${currEvent.repeatingNum>1}">
		           <!-- start of the repeating events-->
		              <td class="table_header_row_left" colspan="2">
		        		<fmt:message key="subject" bundle="${resword}"/>: ${esc.escapeJavaScript(subjectName)}
						<br>
		               ...
		               <!--</td>-->
						...
						"						
							onClick="layersShowOrHide('hidden','Lock_all'); 
								layersShowOrHide('hidden','Event_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>');
								layersShowOrHide('hidden','Lock_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>');
								javascript:setImage('ExpandIcon_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>','images/icon_blank.gif');">X</a>
						 <br>
        ...
        <!-- skip forward/back links: variables represent Table ID for this event, number of occurrences of repeating event, and number to skip to -->
						...
						<c:forEach begin="1" end="${currEvent.repeatingNum}">
						 <a href="javascript:StatusBoxSkip('${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>',<c:out value="${currEvent.repeatingNum}"/>,<c:out value="${rc}"/>);"><c:out value="${rc}"/></a><c:if test="${rc < currEvent.repeatingNum}">|</c:if>
					...
					<tr>
					<td id="Scroll_off_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>_back" class="statusbox_scroll_L_dis" width="20">
					<img src="images/arrow_status_back_dis.gif" border="0">
					</td>

					<td id="Scroll_on_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>_back" width="20" class="statusbox_scroll_L" style="display: none;">  
		            
		           	
		          <!-- scroll back arrow: variables represent Table ID for this event, and number of occurrences of repeating event -->
		              
				    <div id="bt_Scroll_Event_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>_back" style="display: none;">
				    <a href="javascript:StatusBoxBack('${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>',<c:out value="${currEvent.repeatingNum}"/>);">
				    <img src="images/arrow_status_back.gif" border="0"></a></div>
					<div id="bt_Scroll_Event_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>_back_dis">
					<img src="images/arrow_status_back_dis.gif" border="0"></div>
					</td>
					
					<td id="Event_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>_1" valign="top" width="180">
		           ...
		           <!-- the options for this event, will be hidden unless user clicks -->
						<tr id="Menu_on_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>_1" style="display: none">
							<td colspan="2">
								<table border="0" cellpadding="0" cellspacing="0" width="100%">							    		
	             ...
	             <!-- start to get the other events in currEvent.repeatEvents array-->                
		              ...
		              <c:forEach var="reEvent" items="${currEvent.repeatEvents}">
		               <td id="Event_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>_<c:out value="${reNum}"/>" valign="top" width="180" <c:if test="${reNum > 3}"> style="display: none;" </c:if>>
					  <table border="0" cellpadding="0" cellspacing="0" width="100%">
						<tr valign="top">
						<td class="table_header_row" colspan="2">
						<b>
		              ...
		              <!-- the options for this event, will be hidden unless user clicks -->  
		                 ...
		                 <tr id="Menu_on_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>_<c:out value="${reNum}"/>" style="display: none">
							<td colspan="2">
							<table border="0" cellpadding="0" cellspacing="0" width="100%">						    		
					...
					<td id="Scroll_off_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>_next" width="20"  class="statusbox_scroll_R_dis">
					<img src="images/arrow_status_next_dis.gif" border="0">
					</td>

					<td id="Scroll_on_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>_next" width="20"  class="statusbox_scroll_R" style="display: none;">

            <!-- scroll next arrow: variables represent Table ID for this event, and number of occurrences of repeating event -->

					<div id="bt_Scroll_Event_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>_next"><a href="javascript:StatusBoxNext('${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>',<c:out value="${currEvent.repeatingNum}"/>);"><img src="images/arrow_status_next.gif" border="0"></a></div>
            ...
            <a href="javascript:leftnavExpand('Menu_on_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>'); "
							onClick="layersShowOrHide('hidden','Lock_all'); 
								layersShowOrHide('hidden','Event_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>');
								layersShowOrHide('hidden','Lock_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>');
								javascript:setImage('ExpandIcon_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>','images/icon_blank.gif');">X</a></td>
						</c:when>
						<c:otherwise>
	   ...
	   <a href="javascript:leftnavExpand('Menu_on_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>'); 
         ...
         <%--if(! detectIEWindows(navigator.userAgent)){
          "
							...
							javascript:setImage('ExpandIcon_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>','images/icon_blank.gif');"
			  ...
			  "moveObject('Event_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>', event); setImage('ExpandIcon_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>','images/icon_expand.gif');"
				onmouseout=
				"layersShowOrHide('hidden','Event_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>'); setImage('ExpandIcon_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>','images/icon_blank.gif');"
				onClick=
				"layersShowOrHide('visible','Lock_all'); LockObject('Lock_${esc.escapeJavaScript(subjectName)}_<c:out value="${count}"/>_<c:out value="${eblRowCount}"/>', event);">
