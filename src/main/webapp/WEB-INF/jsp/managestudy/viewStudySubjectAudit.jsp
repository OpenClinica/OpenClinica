                            ...
                            <td class="table_header_column">
                                ...
                                the new value is the Status id (both objects in a StudyEventBean) --%>
                                           ...
                                           <c:when test="${studyEventAudit.entityName=='Status'}">
                                                <c:choose>
                                                    <c:when test="${studyEventAudit.oldValue eq 'SCHEDULED' }">scheduled</c:when>
                                                    <c:when test="${studyEventAudit.oldValue eq 'NOT_SCHEDULED' }">not_scheduled</c:when>
                                                    <c:when test="${studyEventAudit.oldValue eq 'DATA_ENTRY_STARTED' }">data_entry_started</c:when>
                                                    <c:when test="${studyEventAudit.oldValue eq 'COMPLETED' }">completed</c:when>
                                                    <c:when test="${studyEventAudit.oldValue eq 'STOPPED' }">stopped</c:when>
                                                    <c:when test="${studyEventAudit.oldValue eq 'SKIPPED' }">skipped</c:when>
                                                </c:choose>
                                           </c:when>
    ...
    <c:choose>
                                    ...
                                    <c:otherwise>
                                        <c:choose>
                                           ...
                                           <c:when test="${studyEventAudit.entityName=='Status'}">
                                                <c:choose>
                                                    <c:when test="${studyEventAudit.newValue eq 'SCHEDULED' }">scheduled</c:when>
                                                    <c:when test="${studyEventAudit.newValue eq 'NOT_SCHEDULED' }">not_scheduled</c:when>
                                                    <c:when test="${studyEventAudit.newValue eq 'DATA_ENTRY_STARTED' }">data_entry_started</c:when>
                                                    <c:when test="${studyEventAudit.newValue eq 'COMPLETED' }">completed</c:when>
                                                    <c:when test="${studyEventAudit.newValue eq 'STOPPED' }">stopped</c:when>
                                                    <c:when test="${studyEventAudit.newValue eq 'SKIPPED' }">skipped</c:when>
                                                </c:choose>
                                           </c:when>
                                           <c:when test="${studyEventAudit.entityName=='Removed' or studyEventAudit.entityName=='Archived' or studyEventAudit.entityName=='Locked' or studyEventAudit.entityName=='Signed'}">
                                              <c:choose>
                            ...
                            <td class="table_header_column">
                                <c:choose>
                                  <c:when test='${eventCRFAudit.auditEventTypeId == 12 or eventCRFAudit.entityName eq "Status"}'>
                                        ...
                                        <c:if test="${eventCRFAudit.oldValue eq 'INITIAL_DATA_ENTRY' }">initial data entry</c:if>
                                        <c:if test="${eventCRFAudit.oldValue eq 'COMPLETED' }">completed</c:if>
                                        <c:if test="${eventCRFAudit.oldValue eq 'NOT_STARTED' }">not started</c:if>
                                    </c:when>
                            ...
                            <td class="table_header_column">
                                <c:choose>
                                    <c:when   test='${eventCRFAudit.auditEventTypeId == 12 or eventCRFAudit.entityName eq "Status"}'>
                                        ...
                                        <c:if test="${eventCRFAudit.newValue eq 'INITIAL_DATA_ENTRY' }">initial data entry</c:if>
                                        <c:if test="${eventCRFAudit.newValue eq 'COMPLETED' }">completed</c:if>
                                        <c:if test="${eventCRFAudit.newValue eq 'NOT_STARTED'}">not started</c:if>
                                    </c:when>
                                    <c:when test="${eventCRFAudit.entityName=='Removed' or eventCRFAudit.entityName=='Archived'}">
                                        <c:choose>
