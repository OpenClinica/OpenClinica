    ...
    <c:when test="${study.status.description != 'removed' && study.status.description != 'auto-removed'}">
        ...
        <c:choose>
            <c:when test="${study.study.studyId>0}">
                <b><fmt:message key="study" bundle="${resword}"/>:</b>&nbsp;<a href="${viewStudy}?id=<c:out value="${study.study.studyId}"/>&viewFull=yes"><c:out value="${study.study.name}"/></a>
                <br><br>
                <b>Site:</b>&nbsp;<a href="${viewStudy}?id=<c:out value="${study.studyId}"/>">
            </c:when>   
            <c:otherwise>
                <b><fmt:message key="study" bundle="${resword}"/>:</b>&nbsp;<a href="${viewStudy}?id=<c:out value="${study.studyId}"/>&viewFull=yes">
            </c:otherwise>
        </c:choose>
        ...
        &nbsp;&nbsp;-&nbsp;<fmt:message key="study_xml" bundle="${resword}"/>:&nbsp;<b><a href="DownloadStudyMetadata?studyId=<c:out value="${study.studyId}"/>"></b><fmt:message key="download" bundle="${resword}"/></a><br/>
