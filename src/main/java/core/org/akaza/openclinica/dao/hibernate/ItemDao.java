...
import core.org.akaza.openclinica.bean.oid.ItemOidGenerator;
...
public class ItemDao extends AbstractDomainDao<Item> {
    ...
    public Item findByOcOIDCrfId(String OCOID, Integer crfId) {
        String query = "select distinct i.* from item i, item_form_metadata ifm, crf_version cv where i.oc_oid= '" + OCOID + "' and i.item_id= ifm.item_id and ifm.crf_version_id=cv.crf_version_id and cv.crf_id=" + crfId;
        org.hibernate.Query q = getCurrentSession().createSQLQuery(query).addEntity(Item.class);
        return ((Item) q.uniqueResult());
    }
