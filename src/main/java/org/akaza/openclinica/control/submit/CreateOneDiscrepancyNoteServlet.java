        ...
        if (errors.isEmpty()) {
            ...
            if(success) {
                ...
                if (dn.getAssignedUserId() > 0 && "1".equals(email.trim())) {
                    ...
                    message.append(respage.getString("email_body_separator"));
                    DiscrepancyNoteBean parentDn= (DiscrepancyNoteBean) dndao.findByPK(dn.getParentDnId());
                    message.append(MessageFormat.format(respage.getString("mailDNParameters1"), String.valueOf(parentDn.getThreadNumber()),dn.getDetailedNotes(), ub.getName()));
                    message.append(respage.getString("email_body_separator"));
    ...
    private String appendPageFileName(String origin, String parameterName, String parameterValue) {
        ...
        if (a.length == 2) {
            if(("&"+a[1]).contains("&"+parameterName+"=")) {
                ...
                if(b.length==2) {
                    result += b[0].substring(1) + "&" + parameter + (b[1].contains("&") ? b[1].substring(b[1].indexOf("&")) : "");
                    return result;
                } else if(b.length>2) {
