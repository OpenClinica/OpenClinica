...
//                scs.setParametersForSite(newStudy);
        ...
        if (currentRole.isMonitor()) {
            response.sendRedirect(request.getContextPath() + "/pages/viewAllSubjectSDVtmp?sdv_restore=true&studyId=" + currentStudy.getStudyId() + "&studyJustChanged=yes&sdv_f_sdvStatus=Ready+to+Verify+%2B+Changed+since+verified");
            return;
