...
-->
    ...
    <xsl:template mode="studyEventData"
        ...
        <xsl:variable name="crfStatusExist" select="odm:FormData/@OpenClinica:Status"/>
        <xsl:variable name="crfVersionIdExist" select="odm:FormData/@OpenClinica:FormLayoutOID"/>
        <xsl:variable name="eventLocationExist" select="@OpenClinica:StudyEventLocation"/>
        <xsl:variable name="eventStartDateExist" select="@OpenClinica:StartDate"/>
        ...
        <xsl:if test="@StudyEventOID = $parentEvent/@StudyEventOID">
            <xsl:choose>
                ...
                <xsl:otherwise><!--
                    ...
                    <xsl:if test="$crfVersionIdExist">
                        <xsl:text>CRF Version ID_</xsl:text>
                        <xsl:value-of select="$E"/>
                        <xsl:value-of select="$eventPosition"/>
                        <xsl:text>_</xsl:text>
                        <xsl:value-of select="$C"/>
                ...
                <xsl:otherwise>
                    <xsl:choose>
                        <xsl:when test="$subjectFormData/node()"><!--
                            ...
                            <xsl:if test="$crfVersionIdExist">
                                <xsl:value-of select="$currentForm/@OpenClinica:FormLayoutOID"/>
                                <xsl:value-of select="$delimiter"/>
                            </xsl:if>
                            <xsl:if test="$crfStatusExist">
                                <xsl:value-of select="$currentForm/@OpenClinica:Status"/>
    ...
    <xsl:template name="createColForRepeatingEvent" match="/odm:ODM/odm:Study/odm:MetaDataVersion/odm:StudyEventDef"
                  mode="createColForRepeatingEvent">
        <xsl:param name="eventOID"/>
        <xsl:param name="eventPosition"/>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:for-each select="odm:FormRef">
                    ...
                    <xsl:variable name="crfVersionIdExist" select="count(//odm:FormData[../@StudyEventOID = $eventOID and @FormOID = $formRefOID and
					@OpenClinica:FormLayoutOID]) &gt; 0"/>
                    ...
                    <xsl:if test="count($allStudyEventDataElements[@StudyEventOID = $eventOID and odm:FormData/@FormOID =
                        ...
                        <xsl:if test="$interviewerNameExist">
                            ...
                            <xsl:choose>
                                <xsl:when
                                        test="$generateIntHeadersList = 'Yes'"><!-- Use special constants here than '_E' for internal processing -->
                        ...
                        <xsl:if test="$crfVersionIdExist">
                            ...
                            <xsl:value-of select="' '"/>
                            <xsl:text>CRF Version ID</xsl:text>
                            <xsl:choose>
                                <xsl:when
                                        test="$generateIntHeadersList = 'Yes'"><!-- Use special constants here than '_E' for internal processing -->
                                    <xsl:value-of select="$itemNameAndEventSep"/>
    ...
    <xsl:template name="createCRFColForRepeatingEvent" mode="createCRFColForRepeatingEvent"
	   ...
	   <xsl:param name="crfVersionExist"/>
        ...
        <xsl:for-each select="odm:FormRef">
            ...
            <xsl:variable name="crfVersionIdExist" select="count(//odm:FormData[../@StudyEventOID = $eventOID and @FormOID = $formRefOID and @OpenClinica:FormLayoutOID  and
            ../@StudyEventRepeatKey = $eventRepeatCnt]) &gt; 0"/>
        ...
        <xsl:choose>
            <xsl:when test="$isEventRepeating = 'Yes'">
                ...
                <xsl:apply-templates select="." mode="createItemDataColForRepeatingEvent">
                    ...
                    <xsl:with-param name="eventRepeatCnt" select="1"/>
                    <xsl:with-param name="MaxEventRepeatKey" select="$MaxEventRepeatKey"/>
                    <xsl:with-param name="isEventRepeating"
                                    select="$isEventRepeating"/><!-- this is just need to pass on to further template which is common to repeating and non-repeating events -->
                    <xsl:with-param name="generateIntHeadersList" select="$generateIntHeadersList"/>
                </xsl:apply-templates>
            </xsl:when>
            <xsl:otherwise>
                <xsl:for-each select="odm:FormRef">
                    ...
                    <xsl:apply-templates mode="formRefToDefTemplateForHeaders"
                        ...
                        <xsl:with-param name="StudyEventRepeatKey"
                                        select="$MaxEventRepeatKey"/><!-- this param is of no use for non-repeating column further when creating the columns -->
                        <xsl:with-param name="generateIntHeadersList" select="$generateIntHeadersList"/>
                    </xsl:apply-templates>
    ...
    <xsl:template name="createItemDataColForRepeatingEvent"
        ...
        <xsl:if test="count($allStudyEventDataElements[@StudyEventOID = $eventOID and @StudyEventRepeatKey =
            ...
            <xsl:for-each select="odm:FormRef">
                ...
                <xsl:apply-templates select="/odm:ODM/odm:Study/odm:MetaDataVersion/odm:FormDef[@OID = $formRefOID]"
    ...
    <xsl:template mode="ItemGrpRefs"
        ...
        <xsl:apply-templates select="/odm:ODM/odm:Study/odm:MetaDataVersion/odm:ItemGroupDef[@OID = $grpOID]"
    ...
    <xsl:template name="studyEventInfoData2">
        ...
        <xsl:for-each select="$tokenizedEventHeaders">
            ...
            <xsl:if test=". != $tokenizedEventHeaders[last()]"><!--not last	-->
                ...
                <xsl:variable name="isColForRepeatingEvent"
                              select="contains($numericStart, '_')"/><!--isColForRepeatingEvent<xsl:value-of select="$isColForRepeatingEvent"/>-->
                            ...
                            <xsl:when test="normalize-space($colEventPosition) = $eventPosition"><!--event matched-->
                                <xsl:choose>
                                    <xsl:when test="$isColForRepeatingEvent"><!-- repeating event-->
                                        ...
                                        <xsl:choose>
                                            <xsl:when
                                                ...
                                                <!--<xsl:text>M</xsl:text>-->
                                                <xsl:value-of select="$matchSep"/><xsl:value-of
                                                        select="position()"/><!--_<xsl:value-of select="@StudyEventRepeatKey"/>-->
                                            </xsl:when>
                                    ...
                                    <xsl:otherwise><!--non repeating event-->
                                        <!--<xsl:text>M</xsl:text>-->
                                        <xsl:value-of select="$matchSep"/><xsl:value-of select="position()"/>
                                    </xsl:otherwise>
                                </xsl:choose>
                            </xsl:when>
                            <xsl:otherwise>
			...
			next token:*<xsl:value-of select="$nextToken"/>*-->
                <xsl:variable name="numericStart">
                    <xsl:choose>
                        ...
                        <xsl:when test="ends-with($nextToken,'CRF Version ID')">
                            <xsl:value-of
                                    select="substring-before($nextToken,concat(concat($delimiter, ' '), 'CRF Version ID'))"/>
                        </xsl:when>
                ...
                <xsl:variable name="isColForRepeatingEvent"
                              select="contains($numericB4_C, '_')"/><!--isColForRepeatingEvent<xsl:value-of select="$isColForRepeatingEvent"/>-->
                <xsl:variable name="colRepeatEventKey">
                ...
                <xsl:variable name="colType">
                    <xsl:choose>
                        ...
                        <xsl:when test="ends-with($currentToken,'CRF Version ID')">
                            <xsl:text>CRF Version ID</xsl:text>
                        </xsl:when>
                ...
                <xsl:variable name="isColForRepeatingGrp"
                              select="contains($numericAfter_C, '_')"/><!--isColForRepeatingGrp<xsl:value-of select="$isColForRepeatingGrp"/>-->
                ...
                <xsl:choose>
                    <xsl:when test="$colType = 'ItemData'"><!--data column -->
                        <xsl:variable name="ifMatch">
                            <xsl:for-each select="$subjectEvents">
                                <xsl:variable name="eventOID"
                                              select="@StudyEventOID"/><!--{eventOID:<xsl:value-of select="$eventOID"/>}-->
                                <xsl:variable name="eventRepeatKey"
                                              select="@StudyEventRepeatKey"/><!--{eventRepeatKey:<xsl:value-of select="$eventRepeatKey"/>}-->
                                <xsl:variable name="eventPosition">
                                    <xsl:for-each select="/odm:ODM/odm:Study[1]/odm:MetaDataVersion/odm:StudyEventDef">
                                        <xsl:if test="@OID = $eventOID">
                                            <xsl:copy-of select="position()"/>
                                    ...
                                    <xsl:when test="$colEventPosition = $eventPosition"><!--{event matched}-->
                                        <xsl:choose>
                                            <xsl:when test="$isColForRepeatingEvent"><!--{repeating event }-->
                                                <xsl:choose>
                                                    <xsl:when
                                                            test="$colRepeatEventKey = $eventRepeatKey"><!--{event repeat match}-->
                                                        <xsl:for-each select="./odm:FormData">
                                                            <xsl:variable name="formOID"
                                                                          select="@FormOID"/><!--{formOID:<xsl:value-of select="$formOID"/>}-->
                                                            <!-- find crf position -->
                                                                ...
                                                                <xsl:when
                                                                    ...
                                                                    <xsl:for-each select="./odm:ItemGroupData">
                                                                        <xsl:variable name="grpOID"
                                                                                      select="@ItemGroupOID"/><!--{grp OID<xsl:value-of select="$grpOID"/>}-->
                                                                        <xsl:variable name="grpRepeatKey"
                                                                                      select="@ItemGroupRepeatKey"/>
                                                                        <xsl:choose>
                                                                            <xsl:when
                                                                                    test="$isColForRepeatingGrp"><!--{grp repeating}{grp RepeatKey:*<xsl:value-of select="$grpRepeatKey"/>*}{colRepeatGrpKey:*<xsl:value-of select="$colRepeatGrpKey"/>*}-->
                                                                                <xsl:choose>
                                                                                    <xsl:when
                                                                                        ...
                                                                                        <xsl:for-each
                                                                                            ...
                                                                                            <xsl:choose>
                                                                                                <xsl:when
                                                                                                    ...
                                                                                                    <xsl:value-of
                                                                                                            select="$matchSep"/><xsl:value-of
                                                                                                            select="$eventOID"/><xsl:value-of
                                                                                                            select="$mValSeparator1"/>
                                                                                                    <xsl:text>_</xsl:text><xsl:value-of
                                                                                                            select="$formOID"/><xsl:value-of
                                                                                                            select="$mValSeparator2"/>
                                                                                                    <xsl:text>_</xsl:text><xsl:value-of
                                                                                                            select="$grpOID"/><xsl:value-of
                                                                                                            select="$mValSeparator3"/>
                                                                                                    <xsl:text>_</xsl:text><xsl:value-of
                                                                                                            select="$itemOID"/><xsl:value-of
                                                                                                            select="$mValSeparator4"/>
                                                                                                    <xsl:text>_</xsl:text><xsl:value-of
                                                                                                            select="$colRepeatEventKey"/><xsl:value-of
                                                                                                            select="$mValSeparator5"/>
                                                                                                    <xsl:text>_</xsl:text><xsl:value-of
                                                                                                            select="$grpRepeatKey"/>
                                                                                                </xsl:when>
                                                                                                <xsl:otherwise>
										...
										<xsl:variable name="matchingCRFRef" select="$matchingEventDef/odm:FormRef[@FormOID = $formOID]"/>-->
                                                    ...
                                                    <xsl:choose>
                                                        <xsl:when
                                                            ...
                                                            <xsl:for-each select="./odm:ItemGroupData">
                                                                <xsl:variable name="grpOID"
                                                                              select="@ItemGroupOID"/><!--grpOID:<xsl:value-of select="$grpOID"/>-->
                                                                <xsl:variable name="grpRepeatKey"
                                                                              select="@ItemGroupRepeatKey"/><!--grpRepeatKey:<xsl:value-of select="$grpRepeatKey"/>-->
                                                                <xsl:choose>
                                                                    ...
                                                                    <xsl:otherwise>
                                                                        ...
                                                                        <xsl:for-each select="./odm:ItemData">
                                                                            <xsl:variable name="itemOID"
                                                                                          select="@ItemOID"/><!--itemOID:<xsl:value-of select="$itemOID"/>-->
                                                                            <xsl:variable name="itemName"
                                                                                          select="//odm:ItemDef[@OID = $itemOID]/@Name"/><!--itemName:<xsl:value-of select="$itemName"/>-->
                                                                            <xsl:choose>
                                                                                <xsl:when
                        ...
                        <xsl:choose>
                            <xsl:when test="contains($ifMatch, $matchSep)">
                                ...
                                <xsl:variable name="itemOID">
                                    <xsl:choose>
                                        <xsl:when
                                                test="$isColForRepeatingEvent"><!-- only event repeating or both event and grp repeating -->
                                            <xsl:value-of
                    ...
                    <xsl:otherwise><!--crf column col type:<xsl:value-of select="$colType"/> -->
                        ...
                        <xsl:variable name="ifMatch">
                            <xsl:for-each select="$subjectEvents">
                                ...
                                <xsl:choose>
                                    <xsl:when
                                            test="normalize-space($colEventPosition) = $eventPosition"><!--event matched-->
                                        <xsl:for-each select="./odm:FormData">
                                            <xsl:variable name="formOID"
                                                          select="@FormOID"/><!--formOID:<xsl:value-of select="$formOID"/>-->
                                            <!-- find crf position -->
                                            ...
                                            <xsl:choose>
                                                <xsl:when
                                                    ...
                                                    <xsl:choose>
                                                        <xsl:when
                                                                test="$isColForRepeatingEvent"><!--col for repeating event -->
                                                            <xsl:choose>
                                                                <xsl:when
                                                                        test="../@StudyEventRepeatKey = normalize-space($colRepeatEventKey)"><!--event repeat key matched-->
                                                                    <!--<xsl:text>M_</xsl:text>-->
                                                                    <xsl:value-of select="$matchSep"/><xsl:value-of
                                                                            select="$eventOID"/><xsl:value-of
                                                                            select="$mValSeparator1"/><xsl:value-of
                                                                            select="$formOID"/><xsl:value-of
                                                                            select="$mValSeparator2"/><xsl:value-of
                                                                            select="../@StudyEventRepeatKey"/>
                                                                </xsl:when>
                                                                <xsl:otherwise><!--<xsl:text>N</xsl:text>-->
                                                                    <xsl:value-of
                                                                            select="$nonMatchSep"/><!--event repeat key mismatch-->
                                                                </xsl:otherwise>
                                                            </xsl:choose>
                                                        </xsl:when>
                            ...
                            <xsl:when test="contains($ifMatch, $matchSep)">
                                ...
                                <xsl:choose>
                                    <xsl:when test="$isColForRepeatingEvent">
                                        ...
                                        <xsl:if test="$colType = 'Interviewer'">
                                            <xsl:choose>
                                                <xsl:when test="$formData/@OpenClinica:InterviewerName">
                                                    <xsl:value-of
                                                            select="$formData/@OpenClinica:InterviewerName"></xsl:value-of>
                                                    <xsl:value-of select="$delimiter"/>
                                                </xsl:when>
                                                <xsl:otherwise>
                                                    <xsl:value-of select="$delimiter"/>
                                        ...
                                        <xsl:if test="$colType = 'InterviewDate'">
                                            <xsl:choose>
                                                <xsl:when test="$formData/@OpenClinica:InterviewDate">
                                                    <xsl:value-of
                                                            select="$formData/@OpenClinica:InterviewDate"></xsl:value-of>
                                                    <xsl:value-of select="$delimiter"/>
                                                </xsl:when>
                                                <xsl:otherwise>
                                                    <xsl:value-of select="$delimiter"/>
                                        ...
                                        <xsl:if test="$colType = 'CRF Version ID'">
                                            <xsl:choose>
                                                <xsl:when test="$formData/@OpenClinica:FormLayoutOID">
                                                    <xsl:value-of
                                                            select="$formData/@OpenClinica:FormLayoutOID"></xsl:value-of>
                                                    <xsl:value-of select="$delimiter"/>
                                                </xsl:when>
                                                <xsl:otherwise>
                                                    <xsl:value-of select="$delimiter"/>
                                        ...
                                        <xsl:if test="$colType = 'Version Name'">
                                            <xsl:choose>
                                                <xsl:when test="$formData/@OpenClinica:Version">
                                                    <xsl:value-of
                                                            select="$formData/@OpenClinica:Version"></xsl:value-of>
                                                    <xsl:value-of select="$delimiter"/>
                                                </xsl:when>
                                                <xsl:otherwise>
                                                    <xsl:value-of select="$delimiter"/>
