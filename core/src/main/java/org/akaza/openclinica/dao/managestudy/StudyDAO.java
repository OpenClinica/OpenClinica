...
public class StudyDAO<K extends String, V extends ArrayList> extends AuditableEntityDAO {
	...
	public StudyBean findByOid(String oid) {
		...
		if (it.hasNext()) {
			...
			if (sb.getParentStudyId()>0){
			    sb.setParentStudyName(findByPK(sb.getParentStudyId()).getName());
			}
			return sb;
		} else {
	...
	public Collection findAllByUser(String username) {
		...
		while (it.hasNext()) {
			...
			// FR 2018-09-20: retrieve and set parentStudyName
			if (eb.getParentStudyId()>0){
			    eb.setParentStudyName(findByPK(eb.getParentStudyId()).getName());
			}
			al.add(eb);
		}
		return al;
	...
	public Collection findAllParents() {
		...
		while (it.hasNext()) {
			StudyBean eb = (StudyBean) this.getEntityFromHashMap((HashMap) it.next());
			// FR 2018-09-20: retrieve and set parentStudyName not sufficient
			// if (eb.getParentStudyId()>0){
			//    eb.setParentStudyName(findByPK(eb.getParentStudyId()).getName());
			// }
			al.add(eb);
