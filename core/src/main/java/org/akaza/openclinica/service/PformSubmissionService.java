...
public class PformSubmissionService {
                ...
                for (int j = 0; j < crfNodeList.getLength(); j = j + 1) {
                    ...
                    if (crfNode instanceof Element) {
                        ...
                        if (!errors.hasErrors()) {
                            ...
                            // Study Event status
                            // update
                            if (getCountCompletedEventCrfsInAStudyEvent(studyEventBean) == getCountCrfsInAEventDefCrf(studyEventDefinitionBean.getId())) {
    ...
    private void getItemFormMetaDataList(ItemDataBean itemDataBean, ItemBean itemGroupBean, EventCRFBean eventCrfBean, CRFVersionBean crfVersionBean) {
