...
xmlns:xs="http://www.w3.org/2001/XMLSchema">
	...
	<xsl:variable name="suffix-resp-options-txt-table" select="'_resp_opts_txt'"/>
	<xsl:template match="/">
...
<!-- 02/21/2010 Replaced the call to substring-before function with custom function substring-before-last for correct results -->
	...
	<xsl:template name="create-table-non-repeating-grp-items">
		...
		<xsl:choose>
			<xsl:when test="$grpItemsCnt &gt; $max-crf-item-cnt-for-single-tbl"><!-- If the items cnt exceeds value of global variable "max-crf-item-cnt-for-single-tbl" ; create another table to store the single-select, multi-select items response 
				option text --> 
	...
	<xsl:template name="create-primary-key-non-repeating-grp-items">
		<xsl:param name="tableName"/>
		<xsl:param name="pFormNameFromVersion"/>
		<!-- @pgawade 07/29/2010 Added the parameter for number of non-grouped CRF items. If this exceeds
			the maximum defined limit, there will be two tables for form data and similar primary keys will be generated
			for both -->  
		<xsl:param name="grpItemsCnt"/>
		...
		<xsl:if test="$grpItemsCnt &gt; $max-crf-item-cnt-for-single-tbl">
			...
			<xsl:variable name="constraint-name-resp-opts-tbl-pre">
				<xsl:value-of select="$pFormNameFromVersion"/><xsl:value-of select="$suffix-resp-options-txt-table"/>
			</xsl:variable>
				...
				<xsl:for-each select="./odm:StudyEventData">
					...
					<xsl:for-each select="./odm:FormData[@FormOID=$formVersionOID]">
						...
						<xsl:if test="$itemGroupData">
							<xsl:choose>
								<xsl:when test="$grpItemsCnt &gt; $max-crf-item-cnt-for-single-tbl"><!-- If the items cnt exceeds max-crf-item-cnt-for-single-tbl; insertb data 
...
<xsl:function name="fun:substring-before-last" as="xs:string" 
 ...
 "/>   
	...
	<xsl:template name="get-multi-select-columns-alter-table-statements">
		...
		<xsl:param name="schemaName"/>
		<xsl:param name="tableName"/>
		<!-- @pgawade 08/28/2010 Changed the parameter from itemOID to itemName -->
		<!--<xsl:param name="itemOID"/>-->
		<xsl:param name="itemName"/>
		...
		<xsl:for-each select="$multiSelectList/OpenClinica:MultiSelectListItem">
			<!-- 02/21/2010 Replaced the invalid SQL characters in the label of multi-select column names -->
			...
			<!-- @ccollins 5/11/10 added special add column function that checks for existing column of same name before adding; as a (temporary?) fix to duplicates bug -->
			<!-- @pgawade 08/28/2010 Corrected the item OID in following statement to item name -->
			<!-- old code
...
-->
	...
	<xsl:template name="get-multi-select-column-name-for-value-selected">
		...
		<xsl:for-each select="$multiSelectList/OpenClinica:MultiSelectListItem">
			<!-- @pgawade 07/29/2010 Corrected the attribute name to get the value of response option from the codelist -->
			<!-- old code
