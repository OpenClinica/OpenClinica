 ...
 * <p>
 * Login forms must present two parameters to this filter: a username and password. The default parameter names to use are contained in the static fields
 * {@link #SPRING_SECURITY_FORM_USERNAME_KEY} and {@link #SPRING_SECURITY_FORM_PASSWORD_KEY}. The parameter names can also be changed by setting the
 ...
 */
public class OpenClinicaAuthenticationProcessingFilter extends AbstractProcessingFilter {
     ...
     * <p>
     * This might be used for example if a postcode/zipcode was required in addition to the password. A delimiter such as a pipe (|) should be used to separate
     * the password and extended value(s). The <code>AuthenticationDao</code> will need to generate the expected password in a corresponding manner.
     * </p>
     * 
     * @param request
