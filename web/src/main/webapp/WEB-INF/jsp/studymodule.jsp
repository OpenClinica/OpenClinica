            ...
            <td>
                ...
                <c:choose>
                    <c:when test="${studyModuleStatus.rule == 1}">
						...
						<a href="${ruleDesignerURL}access?host=${hostPath}&app=${contextPath}&study_oid=${study.oid}&provider_user=${userBean.name}&target=I_001AL_31JANTLORGAN&ruleOid=08FEB1&path=${path}"><img src="../images/bt_EnterData.gif" border="0" alt="<fmt:message key="rule_designer" bundle="${resword}"/>" title="<fmt:message key="rule_designer" bundle="${resword}"/>"/></a>
                    </c:when>
                    <c:when test="${studyModuleStatus.rule == 2}">
