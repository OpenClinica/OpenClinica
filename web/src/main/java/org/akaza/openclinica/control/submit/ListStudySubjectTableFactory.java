...
public class ListStudySubjectTableFactory extends AbstractTableFactory {
	...
	@Override
	// To avoid showing title in other pages, the request element is used to
	...
	protected void configureColumns(TableFacade tableFacade, Locale locale) {
		...
		for (int i = index; i < index + studyGroupClasses.size(); i++) {
			...
			configureColumn(row.getColumn(columnNames[i]), studyGroupClass.getName(),
					new StudyGroupClassCellEditor(studyGroupClass),
		...
		for (int i = index + studyGroupClasses.size(); i < columnNames.length - 1; i++) {
			StudyEventDefinitionBean studyEventDefinition = studyEventDefinitions
					.get(i - (index + studyGroupClasses.size()));
	...
	public void setDataAndLimitVariables(TableFacade tableFacade) {
		...
		Collection<StudySubjectBean> items = getStudySubjectDAO().getWithFilterAndSort(getStudyBean(), subjectFilter,
		...
		for (StudySubjectBean studySubjectBean : items) {
			...
			for (StudyEventBean studyEventBean : allStudyEventsForStudySubject) {
				if (allStudyEventsForStudySubjectBySedId.get(studyEventBean.getStudyEventDefinitionId()) == null) {
					...
					allStudyEventsForStudySubjectBySedId.get(studyEventBean.getStudyEventDefinitionId())
							.add(studyEventBean);
				}
			...
			for (StudyGroupClassBean studyGroupClass : getStudyGroupClasses()) {
				subjectGroupMapBean = getSubjectGroupMapDAO()
						.findAllByStudySubjectAndStudyGroupClass(studySubjectBean.getId(), studyGroupClass.getId());
			...
			for (StudyEventDefinitionBean studyEventDefinition : getStudyEventDefinitions()) {
				...
				List<StudyEventBean> studyEvents = allStudyEventsForStudySubjectBySedId
						.get(studyEventDefinition.getId());
	...
	private ArrayList<StudyEventDefinitionBean> getStudyEventDefinitions() {
		if (this.studyEventDefinitions == null) {
			if (studyBean.getParentStudyId() > 0) {
				studyEventDefinitions = getStudyEventDefinitionDao()
						.findAllActiveByParentStudyId(studyBean.getParentStudyId());
			} else {
	...
	public class SubjectEventStatusFilterMatcher implements FilterMatcher {
		public boolean evaluate(Object itemValue, String filterValue) {
			...
			String filter = StringUtils.lowerCase(String.valueOf(filterValue));// .trim().replace("
																				// ",
																				// "_");
	...
	private class StudyEventDefinitionMapCellEditor implements CellEditor {
		...
		public Object getValue(Object item, String property, int rowcount) {
			...
			// FR 2018-09-23: find lowest subjectEventStatus assuming lowest
			// subjectEventStatus.id is lowest status
			SubjectEventStatus minSubjectEventStatus = subjectEventStatus;
			for (Iterator<StudyEventBean> iterator = studyEvents.iterator(); iterator.hasNext();) {
				StudyEventBean studyEventBean = (StudyEventBean) iterator.next();
				SubjectEventStatus subjectEventStatus = studyEventBean.getSubjectEventStatus();
				// FR: ignore stopped (5) and skipped (6)
				if (5 != subjectEventStatus.getId() && 6 != subjectEventStatus.getId()) {
					// FR: find min Status
					if (subjectEventStatus.getId() <= minSubjectEventStatus.getId()) {
						minSubjectEventStatus = subjectEventStatus;
					}
				}
			}
			...
			// set found SubjectEventStatus...
			url.append("<img src='" + imageIconPaths.get(minSubjectEventStatus.getId())
					+ "' border='0' style='position: relative; left: 7px;'>");
			url.append(getCount());
			...
			if (studySubjectId != null) {
				...
				if (getCurrentRole().getRole() != Role.MONITOR) {
					if (getStudyBean().getStatus() == Status.AVAILABLE
							...
							&& getCurrentRole().getRole() != Role.RESEARCHASSISTANT2) {
	...
	private String participateStatus(StudySubjectBean studySubjectBean) {
		...
		String participateFormStatus = getStudyParameterValueDAO()
				.findByHandleAndStudy(pStudy.getId(), "participantPortal").getValue();
	...
	private String pManageStatus(StudySubjectBean studySubjectBean) throws Exception {
		...
		StudyBean study = (StudyBean) studyDAO.findByPK(studySubjectBean.getStudyId());
		StudyBean pStudy = getParentStudy(study.getOid());
		String pManageStatus = participantPortalRegistrar.getCachedRegistrationStatus(pStudy.getOid(), session)
				.toString(); // ACTIVE
	...
	private String viewStudySubjectLinkBuilder(StudySubjectBean studySubject) {
		...
		actionLink.img().name("bt_View1").src("images/bt_View.gif").border("0").alt(resword.getString("view"))
				.title(resword.getString("view")).append("hspace=\"2\"").end().aEnd();
		actionLink.append("&nbsp;&nbsp;&nbsp;");
	...
	private String viewParticipateBuilder(StudySubjectBean studySubject) throws Exception {
		...
		actionLink.img().name("bt_Participate1").src("images/bt_Ocui.gif").border("0")
				.alt(resword.getString("connect_participant")).title(resword.getString("connect_participant"))
	...
	private String removeStudySubjectLinkBuilder(StudySubjectBean studySubject) {
		HtmlBuilder actionLink = new HtmlBuilder();
		actionLink.a().href("RemoveStudySubject?action=confirm&id=" + studySubject.getId() + "&subjectId="
	...
	private String signStudySubjectLinkBuilder(StudySubjectBean studySubject) {
		...
		actionLink.img().name("bt_Sign1").src("images/icon_Signed.gif").border("0").alt(resword.getString("sign"))
				.title(resword.getString("sign")).append("hspace=\"2\"").end().aEnd();
		actionLink.append("&nbsp;&nbsp;&nbsp;");
		return actionLink.toString();
	...
	private String reAssignStudySubjectLinkBuilder(StudySubjectBean studySubject) {
		...
		actionLink.img().name("bt_Reassign1").src("images/bt_Reassign.gif").border("0")
				.alt(resword.getString("reassign")).title(resword.getString("reassign")).append("hspace=\"2\"").end()
				.aEnd();
		actionLink.append("&nbsp;&nbsp;&nbsp;");
		return actionLink.toString();
	...
	private String restoreStudySubjectLinkBuilder(StudySubjectBean studySubject) {
		HtmlBuilder actionLink = new HtmlBuilder();
		actionLink.a().href("RestoreStudySubject?action=confirm&id=" + studySubject.getId() + "&subjectId="
				+ studySubject.getSubjectId() + "&studyId=" + studySubject.getStudyId());
		...
		actionLink.img().name("bt_Restore1").src("images/bt_Restore.gif").border("0").alt(resword.getString("restore"))
				.title(resword.getString("restore")).append("hspace=\"2\"").end().aEnd();
		return actionLink.toString();
	...
	private String eventDivBuilder(SubjectBean subject, Integer rowCount, List<StudyEventBean> studyEvents,
		...
		eventDiv.div().id("Event_" + studySubjectLabel + "_" + sed.getId() + "_" + rowCount).style(
				"position: absolute; visibility: hidden; z-index: 3;width:" + divWidth + "px; top: 0px; float: left;")
				.close();
		eventDiv.div().styleClass("box_T").close().div().styleClass("box_L").close().div().styleClass("box_R").close()
				.div().styleClass("box_B").close().div().styleClass("box_TL").close().div().styleClass("box_TR").close()
				.div().styleClass("box_BL").close().div().styleClass("box_BR").close();
	...
	private void repeatingEventDivBuilder(HtmlBuilder eventDiv, SubjectBean subject, Integer rowCount,
		...
		// FR 2018-09-23: make view with icons switchable (maybe needed in
		// future)
		boolean showicon = true;
		for (int i = 1; i <= studyEventsSize; i++) {
			eventDiv.ahref(
					"javascript:StatusBoxSkip('" + studySubjectLabel + "_" + sed.getId() + "_" + rowCount + "',"
							+ studyEventsSize + "," + i + ");",
					// FR 2018-09-23: add icon to popup of repeatable events in
					// ListStudySubjects
					addIconTag(studyEvents.get(i - 1).getSubjectEventStatus(), String.valueOf(i), showicon));
			// no delimiter needed if icons are used
			if (i < studyEventsSize && !showicon) {
				eventDiv.append("|");
			}
		}
		// if (showicon){
		...
		eventDiv.a().href("javascript:StatusBoxBack('" + studySubjectLabel + "_" + sed.getId() + "_" + rowCount + "',"
		...
		eventDiv.div().id("bt_Scroll_Event_" + studySubjectLabel + "_" + sed.getId() + "_" + rowCount + "_next")
				.close();
		eventDiv.a().href("javascript:StatusBoxNext('" + studySubjectLabel + "_" + sed.getId() + "_" + rowCount + "',"
				+ studyEventsSize + ");").close();
		...
		eventDiv.td(0).styleClass("table_cell_left").colspan(String.valueOf(studyEventsSize)).close().append("<i>")
				.append(click_for_more_options).append("</i>").tdEnd();
		eventDiv.trEnd(0);
	...
	// FR 2018-09-23: generate icon-tags for popup on ListStudySubjects
	private String addIconTag(SubjectEventStatus subjectEventStatus, String string, boolean showicon) {
		String iconTag = string;

		if (showicon) {
			String iconPath = "images/icon_blank.gif";
			String state = "NA";
			iconPath = imageIconPaths.get(subjectEventStatus.getId());
			state = subjectEventStatus.getName();
			iconTag = "<span class='eventcount_withicon'>" + "<div class='eventcount_withicon_align'><!-- " + state
					+ "-->" + string + "<img class='eventcount_withicon_img' src='" + iconPath + "' border='0'>"
					+ "</div>" + "</span>";
		}
		return iconTag;
	}
	...
	private void singleEventDivBuilder(HtmlBuilder eventDiv, SubjectBean subject, Integer rowCount,
		...
		SubjectEventStatus eventStatus = studyEvents.size() == 0 ? SubjectEventStatus.NOT_SCHEDULED
				: studyEvents.get(0).getSubjectEventStatus();
		...
		eventDiv.trEnd(0);
		eventDiv.tr(0).id("Menu_off_" + studySubjectLabel + "_" + sed.getId() + "_" + rowCount).style("display: all")
				.close();
		eventDiv.td(0).styleClass("table_cell_left").colspan("2").close().append("<i>").append(click_for_more_options)
				.append("</i>").tdEnd();
		eventDiv.trEnd(0);

		eventDiv.tr(0).id("Menu_on_" + studySubjectLabel + "_" + sed.getId() + "_" + rowCount).style("display: none")
				.close();
		eventDiv.td(0).colspan("2").close();
		eventDiv.table(0).border("0").cellpadding("0").cellspacing("0").width("100%").close();
	...
	private void updateStudyEventLinkBuilder(HtmlBuilder builder, Integer studySubjectId, String studyEventId,
			String edit) {
	...
	private void createNewStudyEventLinkBuilder(HtmlBuilder builder, Integer studySubjectId,
			StudyEventDefinitionBean sed, String schedule) {
	...
	private void repeatingIconLinkBuilder(HtmlBuilder builder, String studySubjectLabel, Integer rowCount,
		...
		String href2 = "javascript:leftnavExpand('Menu_off_" + studySubjectLabel + "_" + sed.getId() + "_" + rowCount
				+ "'); ";
		String onmouseover = "moveObject('Event_" + studySubjectLabel + "_" + sed.getId() + "_" + rowCount
				+ "', event); ";
	...
	private void iconLinkBuilder(HtmlBuilder builder, String studySubjectLabel, Integer rowCount,
			List<StudyEventBean> studyEvents, StudyEventDefinitionBean sed) {
		String href1Repeating = "javascript:ExpandEventOccurrences('" + studySubjectLabel + "_" + sed.getId() + "_"
				+ rowCount + "'," + studyEvents.size() + "); ";
		String href1 = "javascript:leftnavExpand('Menu_on_" + studySubjectLabel + "_" + sed.getId() + "_" + rowCount
				+ "'); ";
		String href2 = "javascript:leftnavExpand('Menu_off_" + studySubjectLabel + "_" + sed.getId() + "_" + rowCount
				+ "'); ";
	...
	private void linkBuilder(HtmlBuilder builder, String studySubjectLabel, Integer rowCount,
			List<StudyEventBean> studyEvents, StudyEventDefinitionBean sed) {
		String href1 = "javascript:leftnavExpand('Menu_on_" + studySubjectLabel + "_" + sed.getId() + "_" + rowCount
				+ "'); ";
		String href2 = "javascript:leftnavExpand('Menu_off_" + studySubjectLabel + "_" + sed.getId() + "_" + rowCount
